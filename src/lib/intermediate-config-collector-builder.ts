...
import {
  IntermediateConfigCollector,
  IntermediateExtendsEntry,
} from './interfaces/intermediate-config-collector';
...
export const addConfigToCollector = (
  libConfig: LibraryConfig,
  collector: IntermediateConfigCollector | null = null
): IntermediateConfigCollector => {
  if (collector === null) {
  ...
  const dependencies = sortBy<string>(
